...
class Parameters:
    ...
    def print_params(self):
        ...
        print("     warmstart:", self.warmstart if self.warmstart else "(none)")
        print()
    ...
    def print_usage(self):
        ...
        print("    -warmstart <file>      : MIP start file, one 'weapon target shots' line per")
        print("                             suggested assignment (default: none; incompatible")
        print("                             starts are repaired or ignored by CPLEX).")
        print("")
    ...
    def read_args(self, args) -> bool:
        ...
        while i < len(sys.argv):
            if sys.argv[i] == "-approach":
                ...
                i += 2
            elif sys.argv[i] == "-warmstart":
                self.warmstart = sys.argv[i + 1]
                print("Warmstart file set to %s" % self.warmstart)
                i += 2
...
def apply_warmstart(wta, path: str):
    """M3 extension (-warmstart): register a CPLEX MIP start from a file with
    one 'weapon target shots' line per suggested assignment (targets are local
    indices as written by dwta.wave_runner.write_wave_instance).

    Only the branch-and-adjust formulation exposes integer shot variables
    x(i, j); for other formulations the start is skipped. All unmentioned x
    variables are set to 0 so CPLEX can complete the continuous part of the
    start. Infeasible or partial starts are repaired or discarded by CPLEX
    itself - the solving logic is untouched."""
    xvars = getattr(wta, "x", None)
    if not xvars or not all(isinstance(k, tuple) and len(k) == 2 for k in xvars):
        print("Warmstart skipped: formulation has no x(i,j) shot variables")
        return
    entries = {}
    with open(path, "r") as f:
        for line in f:
            ...
            if len(parts) != 3:
                continue
            i, j, shots = int(parts[0]), int(parts[1]), int(parts[2])
            if (i, j) in xvars:
                entries[xvars[i, j]] = float(shots)
    if not entries:
        print("Warmstart skipped: no usable 'weapon target shots' entries in %s"
              % path)
        return
    values = dict((v, 0.0) for v in xvars.values())
    values.update(entries)
    wta.model.MIP_starts.add(
        cplex.SparsePair(ind=list(values.keys()), val=list(values.values())),
        wta.model.MIP_starts.effort_level.solve_fixed, "warmstart")
    print("Warmstart loaded: %s (%d suggested assignments, %d x variables total)"
          % (path, len(entries), len(values)))
...
def main():
    ...
    print("", flush=True)
    if params.warmstart:
        apply_warmstart(wta, params.warmstart)
    wta.solve()
    print("")
